    ...
    def _parseClippings(self, allClippings):
        ...
        for eachClipping in allClippings:
            ...
            if len(eachClipping) >= 3:
                ...
                books[title]["highlights"].append((clipping, page, location))
            else:
        ...
        for bookName in books:
            ...
            self.addBookToNotion(
                bookName, author, highlightCount, d, aggregatedText)
            print("✓ Book", bookCounter, "/", bookCount, "\t", highlightCount,
                  "\tnotes / highlights has added successfully, from:", bookName, "("+author+")")
            bookCounter += 1

        print(newLine + "× Passed", passedClippingCount,
              "bookmark or unsupported clippings.", newLine)
...
def generateBookCovers(cv):
    # get all the rows in that table
    ...
    for record in all_rows:
        if record.cover == None:
            ...
            if result != None:
                record.cover = result
                print("✓ Book Cover", coverCounter, "/", coverCount,
                      " has been sent, for", record.title)
            else:
                record.cover = no_cover_img
                print("× Book Cover", coverCounter, "/", coverCount,
                      " coulnd't be found. Please replace the placeholder imge with original bookcover manually for", record.title)
        else:
            print("~ Book Cover", coverCounter, "/", coverCount,
                  " is already existing, for", record.title)
        coverCounter += 1


# gets the client bypassing the token
client = NotionClient(token_v2=NOTION_TOKEN)
cv = client.get_collection_view(NOTION_TABLE_ID)
allRows = cv.collection.get_rows()
# prints all the views present in that table
